...
void setup() {
  ...
  Blynk.email("Subject: Irrigo notificaion", "Irrigo is just restarted.");

  if (isFirstConnect) {
    //Blynk.syncAll();
    Blynk.syncVirtual(V10, V20, V30);
    //Serial.println("mi sono sincronizzato");
    terminal.println("");
    terminal.println("");
    terminal.println("***********************************");
    terminal.println("|    Irrigo starting & syncing    |");
    terminal.println("***********************************");
    terminal.flush();
...
/*BLYNK_CONNECTED() {                           // Server values sync at 1 connection ?????
  if (Blynk.connected()) {
    if (isFirstConnect) {
      ...
      //Serial.println("mi sono sincronizzato");
      terminal.println("");
      PrintLogString("  Data e ora attuale");
      Serial.println("  Data e ora attuale");
      terminal.println("");
      terminal.println("************************************");
...
BLYNK_WRITE(V1)                                 // A-Zone elettrovalve manual open/close
{
  if (manual == 0) {                            // current is mode AUTO
    ...
    PrintLogString("-Modo MANUALE-A/B/C CHIUSE");
    ...
    if (param.asInt() == 1) {                            // If OPEN acivated
      ...
      PrintLogString("-B/C CHIUSE-A APERTA");
    }
  }
...
BLYNK_WRITE(V2) {                            // B Zone manual open/close
  if (manual == 0) {                         // current is mode auto
    ...
    if (param.asInt() == 1) {                             // If OPEN acivated
      ...
      PrintLogString("-A/C CHIUSE-B APERTA");
    }
  }
  else {                                                  // current is mode MANUAL
    if (param.asInt() == 1) {                             // If OPEN acivated
...
BLYNK_WRITE(V3) {                            // C Zone manual open/close
  if (manual == 0) {                         // current is mode auto
    ...
    if (param.asInt() == 1) {                            // If OPEN acivated
      ...
      PrintLogString("-A/B CHIUSE-C APERTA");
    }
  }
  else {                                                 // current is mode MANUAL
    if (param.asInt() == 1) {                            // If OPEN acivated
...
BLYNK_WRITE(V101) {                                           // SW reset
  if (param.asInt() == 1) {
    ...
    terminal.print(Time);
    terminal.println(" - Irrigo SW restart received");
    terminal.println("!*!*!*!*!*!*!*!*!*!*!*!*!*!*!*!*!*!");
    terminal.flush();
...
void loop()
{
  ArduinoOTA.handle();
  if (!updating) {
    //    Serial.println("mo Blinko");
    Blynk.run();
    //    Serial.println("mo timero");
    timer.run();
    digitalWrite(LED_ROSSO, manual);
  } else {
...
void PrintLogString(String S)
{
  ...
  /*tweetstr = Date;
    tweetstr = tweetstr + ":";
    tweetstr = tweetstr + Time;
    tweetstr = tweetstr + " - ";
    tweetstr = tweetstr + S;
    Blynk.tweet(tweetstr);*/
}
/*
